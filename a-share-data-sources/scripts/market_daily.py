...
MARKET_LAST_RUN_PATH = STATE_DIR / "market_daily_last_run.json"
...
def _summarize_result(result) -> dict[str, Any]:
    return {
        ...
        "error": result.error,
        "failed_attempts": [
            {"symbol": attempt.symbol, "error": attempt.error, "record_count": attempt.record_count}
...
def run_market_daily(config: MarketDailyConfig) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "include_momentum": config.include_momentum,
        "symbol_refresh": symbol_refresh,
        "last_run_path": str(MARKET_LAST_RUN_PATH),
    }
    if config.plan_only:
        payload["ok"] = True
    ...
    if not selected:
        ...
        payload["finished_at"] = now_iso()
        save_json(MARKET_LAST_RUN_PATH, payload)
        return payload
    result = update_market(selected, include_momentum=config.include_momentum)
    summary = _summarize_result(result)
    payload.update({"ok": bool(result.ok), "finished_at": now_iso(), "record_count": summary["record_count"], "result": summary})
    save_json(MARKET_LAST_RUN_PATH, payload)
    return payload


def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stable market data daily facade.")
    ...
    parser.add_argument("--skip-momentum", action="store_true", help="Fetch realtime quotes only; skip 5-day momentum.")
