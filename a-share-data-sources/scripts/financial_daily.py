...
FINANCIAL_LAST_RUN_PATH = STATE_DIR / "financial_daily_last_run.json"
DEFAULT_BATCH_SIZE = 100


@dataclass
class FinancialDailyConfig:
...
def run_financial_daily(config: FinancialDailyConfig) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "last_run_path": str(FINANCIAL_LAST_RUN_PATH),
    }
    if config.plan_only:
        payload["ok"] = True
    ...
    if not selected:
        ...
        payload["finished_at"] = now_iso()
        save_json(FINANCIAL_LAST_RUN_PATH, payload)
        return payload
    results = update_financial(selected)
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stable financial data daily facade.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        payload = run_financial_daily(
            FinancialDailyConfig(
